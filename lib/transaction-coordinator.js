...
//        console.log('transactionPacketsData',transactionPacketsData)
   ...
   //required for balance payouts
    ...
    async submitMiningSolution(minerAddress,solution_number,challenge_digest,challenge_number,resending){
     ...
     var mintMethod = this.mintHelperContract.methods.proxyMergeMint(solution_number,challenge_digest, this.mergeMintTokens);
     console.log( 'mintMethod executed!');
	...
	console.log('txData',txData);     	
      ...
      //var gas = await this.web3.eth.getGasPrice()*1.2
            //var gas_hex = web3Utils.toHex(web3Utils.toWei(gas.toString(), 'gwei'));

var gas = await this.web3.eth.getGasPrice()*1.2;
console.log('gas price from network',gas);
var cfgGas = this.poolConfig.solutionGasPriceWei*1000000000; //gwei = 1000000000 wei
